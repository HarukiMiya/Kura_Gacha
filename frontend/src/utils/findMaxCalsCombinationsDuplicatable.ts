...
export const findMaxCalsCombinationsDuplicatable = (uniqueCombinations: UniqueCombination[], desiredPrice: number): MaxCombsWithCals => {
    // Unbounded Knapsack
    ...
    for (let j: number = 0; j <= desiredPrice; j++) {
        if (initItem.item_price <= j) {
            dp[0][j] = Math.floor(j / initItem.item_price) * initItem.item_kcal;
        }
    }
    ...
    for (let i = 1; i < n; i++) {
        for (let j = 1; j <= desiredPrice; j++) {
            ...
            const skip = dp[i - 1][j];
            const include = (remainingPrice >= 0) ? currentItem.item_kcal + dp[i][remainingPrice] : 0;
